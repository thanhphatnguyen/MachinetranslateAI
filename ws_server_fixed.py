...
class SonioxRealtimeTranslationSTT(FrameProcessor):
    def __init__(self, api_key: str, translate_type: str = "one_way", lang_a: str = "en", lang_b: str = "vi", enable_diarization: bool = False, extra_context: dict = None, on_translation=None):
        ...
        # Thêm biến lưu ngôn ngữ đích hiện tại
        self.current_target_lang = lang_b
    ...
    async def _flush_sentence(self):
        ...
        if bot_text or user_text:
            if bot_text:
                ...
                logger.info(f"Soniox Translated ({self.current_target_lang}) -> Piper TTS: {bot_text}")
                
                await self.push_frame(LLMFullResponseStartFrame())
                
                # Nhúng mã ngôn ngữ vào metadata để Lớp lọc (LanguageFilter) nhận diện
                text_frame = TextFrame(bot_text)
                text_frame.metadata = {"target_lang": self.current_target_lang}
                await self.push_frame(text_frame)
                
                await self.push_frame(LLMFullResponseEndFrame())

            if self._on_translation:
                speaker_label = f"Speaker {self._current_speaker}" if self._enable_diarization else "Speaker 1"
                await self._on_translation(bot_text, speaker_label, user_text, True)
        ...
        try:
            async for message in self._ws:
                ...
                for token in tokens:
                    ...
                    # Xác định ngôn ngữ đích dựa trên ngôn ngữ nhận diện được (Two-Way)
                    if token_lang and self._type == "two_way":
                        if token_lang == self._lang_a:
                            self.current_target_lang = self._lang_b
                        elif token_lang == self._lang_b:
                            self.current_target_lang = self._lang_a
...
async def start_pipecat_session(config: dict, sdp_offer: str):
    ...
    if app_mode == "gemini_live":
        ...
        async def _send_transcript(text: str, speaker: str, source_text: str, is_final: bool):
            try:
                msg = {
                    "type": "pro_translate",
                    "data": {
